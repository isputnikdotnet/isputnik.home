...
An Inbox is an ordinary gallery library with one switch on. Create one the
usual way ([Libraries](libraries.md)), pointing it at an empty folder inside a
storage container, then open its settings and, on the **Access** tab, turn on
