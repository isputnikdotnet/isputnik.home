...
| **Memories** | "On this day" — the same date in previous years. |
...
**Memories** is "On this day": photos taken on today's date in earlier years.
It appears in the side menu only on days that have some.
...
When an admin has turned on **Photo place names** ([Control panel → Settings → Maps](control-panel.md#maps)),
