...
The header action row exposes **Likes** — a per-user, whole-book save (♥), the same toggle as the player's Likes button and the Likes page (formerly labelled "Bookmark", "My List", then "Likes") — and a **More options** menu with Edit metadata, Mark finished, **Reset progress** (clears playback progress, mirroring the player's menu action), Download, and Share.
