...
- **Changing the login email** (`/api/profile/email`) is gated the same way: with MFA on it needs the password **and** a current second factor. The email is where the email-MFA method delivers codes, so a password-only change would let a thief with a live session repoint both the login identity and the factor. The check runs after the no-op/conflict checks, so a rejected change never burns a backup code. A user without MFA still changes it with the password alone (the old and new addresses are both emailed on any change).
