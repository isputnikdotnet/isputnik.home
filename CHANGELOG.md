...
- **Step-families are drawn.** A parent's other partners now sit beside that parent on the chart, with the children they had together on this person's row; the profile's Relationships tab lists them as **step-parents** and **half-brothers and half-sisters**, each saying which parent they share. Before, a child of a remarried parent met them nowhere.
- **Who counts as living, better judged.** A person whose child was born 85 or more years ago is no longer treated as living — so old ancestors nobody has dates for show their details without being marked deceased one by one. A bare birth year is read as any day of that year, so someone born "1926" stays private until the whole year is past. The profile header follows the same rule: no more "Living · Age 130" for someone marked deceased, and a dated death reads "Died aged 87".
- **A profile's facts fit.** Born and Died each take a row with date and place together, in one column that wraps — the birthplace "Petrovskoe estate, Borovichi uyezd, Novgorod Governorate" was cut to six letters before, and "Married to …" with it. A missing fact says "Not recorded" quietly instead of "Unknown" in bold. **Died** is always the last row of the timeline.
- **One single-parent family per person.** Adding a parent to a second child, or a child under "just Anna", made a new family each time, so the children never became siblings. The family that exists is reused, and deleting a partner folds the survivor's family into their existing one.
- **Partners in the right words.** A dissolved marriage says **Former wife**; a marriage ended by death still says **Wife**, dated from the wedding to the death — "Widowed · 1910 – 1919" instead of "Widowed · since 1910".
- **Privacy at the edges.** A branch editor's GEDCOM export keeps a private person's marriage as a couple only: no dates, no divorce, no citations. A redacted marriage's sources stay off the other spouse's Sources tab. Editors can only change families with someone of theirs in them, and only attach photos they can open themselves.
- **Smaller things.** Single-word names gather under one **No family name** card on Families instead of each being a family. The map says how many places have no pin at the top of the page, where it matters. A biography pasted with hard line breaks reads as sentences again. Source chips name the kind of event, with the event's title under the source. Notes under a profile name the person. The import dialog is titled for what it takes — a GEDCOM or a package — and Settings opens on **Starting person**. On a phone the portrait is smaller, so the name stays on the first screen.
...
- **A person in the Gallery now has an address of their own** (`/gallery/people/…`), so their photos can be linked to and reopened directly.
...
- **Menus open from the bottom of the screen, where your thumb is.** Choosing a library, sorting a shelf, the ⋮ beside a book or a photo, filters — all of them now slide up from the bottom with the page dimmed behind, instead of a small menu dropped somewhere near the top. **Help and Info can be reached on a phone at last**: they live at the foot of the Profile menu, which is now your things (Likes, Bookmarks, Quotes, Collections), then For you with a count, Profile, Control panel and Logout.
- **The front page starts with what is happening, not with chrome.** A greeting and a bell that says how many things are waiting replace the logo and an "Online" badge that was almost always green; "What's new" is one line; and the rows waiting on you carry round buttons with the message underneath, instead of buttons that broke across two lines.
- **A new album, slideshow, story, chapter, family member or note all look the same on the front page** — a cover, who did what and when, and the name of the thing in full. They used to be one-line sentences that cut off exactly where the name began.
- **Your shelves behave like an app.** The A–Z strip is back on a phone as a row you swipe, covers can replace the list, picking several books works, and each row fits more: the author and the running time on one line, the progress bar with its percentage, a tick on the cover of a book kept offline. Saving for offline moved into the ⋮ menu, which opens headed by the book it belongs to.
- **A book's page leads with the one thing you came for.** A full-width **Continue listening** or **Read** under the title, a smaller cover so the title and that button fit the first screen, and everything else — save offline, edit, download, send, delete — as words in a menu rather than six unlabelled icons.
- **The Gallery is a wall of photos again.** They run edge to edge, nine to a screen; a day of one photo fills its row instead of leaving two thirds empty; and the date is all a day carries — sharing and picking belong to **Select**. In the viewer, swipe between photos, tap the photo to hide everything but the photo, and reach Like, Details, Send to and Add to album along the bottom. Folders, albums and slideshows come two to a row instead of one a screen.
- **Reading turns by tapping the page** — the right edge forward, the left edge back — so the arrows no longer sit on top of the words. Listening drops the volume slider your phone already has on its side, and gives the room to speed and the sleep timer.
- **The family tree opens where you can read it.** It used to fit the whole tree into the width of a phone, which left the names four pixels tall; it now opens around the person in focus, and the button that shows the whole tree is still there.
- **A story chapter opens at its beginning.** Tapping Day 2 from the foot of a story used to drop you halfway into it. The same fix reached Profile, where opening a tab such as Devices left the row of tabs showing none of them as chosen.
...
- **The Dashboard's row of views became pages.** The Dashboard itself is the server's health. **Activity** and **Library statistics** are pages beside it under Overview, **Sign-ins** and **Sign-in locations** moved to Security, and **Tasks** leads Maintenance. They were buttons in a second row under the page, so Back skipped over them and none could be opened in a new tab. Now each has its own address.
- **Duplicate cleanup and Missing photos are under Maintenance, Quotes and Maps under Settings.**
- **Reader access moved to your Profile, and every member can use it now.** The tokens that let KOReader, Moon+ Reader or Thorium open your ebooks belong to the person who makes them, but the only page for them was in the control panel, where only administrators could go. Profile has a **Reader access** tab of its own.
- **The sidebar link says Control panel.** It said Settings, and inside the panel there was a second Settings that went somewhere else. Inside the panel it steps aside; **Home** is the way back out.
- **Search lands on the setting, not just its page.** Searching for password policy, deletion protection or a map feature now scrolls to that card and flashes it once, instead of leaving you at the top of a long page. The arrows on Security's Overview land on the card they grade the same way.
- **Pages lead to the next step, already narrowed.** A running scan offers **Follow on Tasks**, opened on that library's tasks. A failed task links to its library and to Scheduled jobs. A cleanup that removed copies links to exactly those copies in the Recycle Bin. A sign-in address you are looking into offers its entries in Logs, **Block this address** with the address filled in, and the lockout settings. The links to Storage from Stories, Maps, Backup, the family tree and Storage contents now scroll to the card they mean.
- **On a phone the control panel is one row.** Home, a button that names the page you are on, and search. The button opens every section and page in one list, scrolled to where you are, instead of a strip that scrolled sideways past most of its own links.
- **Old links still work.** Every address the moved pages used to have, including Dashboard links that named a view, opens the page that took it over.

## 4.14.0 — Say it about a photo the way Review mode does

- **The photo panel now edits a photo the way Review mode does.** Review mode had the easier ways in — towns offered as you type, a proper note editor with dictation, and people to pick from rather than names to remember. The panel beside a photo, where most photos actually get edited, still had bare text boxes. Now both work the same.
- **Where? offers the place as you type.** Click **Edit** beside the place and start typing: towns and villages from this server's own place list appear under the box, and **Search online** asks OpenStreetMap only when you press it. Picking one writes a short name, such as "Ratomka, Belarus", and puts a photo that is not on the map yet on it. A photo that already has a pin keeps it — moving a pin is the Map tab's job, so a precise camera location is never swapped for a town centre by accident.
- **The Map tab offers places as you type too.** **Mark where it was taken** and **Move the pin** used to find nothing until **Search** was pressed. Towns now appear as you type and pick straight onto the map, and the **Set location** dialog for a selection of photos works the same way.
- **What do you remember? replaces Description and Recordings.** One section holds the words and the recordings, with **Add note** and **Add recording** under them. **Add note** opens Review mode's note dialog — bold, lists and quotes, and **Speak instead** to dictate — instead of a small text box. Arrow keys pressed in a dialog opened over a photo no longer turn to the next photo behind it.
- **Adding a person shows who you are picking.** The **+** under People opens a search box listing the people seen most often, each with their face and how many photos they are in, so two Annas can be told apart. Names starting with what you type come first, a name nobody has yet is offered as a new person, ↑ ↓ and Enter work, and the box stays open so a whole group can be tagged in one go. It replaces the browser's plain light list of names.
- **Adding a tag works the same way.** The **+** under Tags lists the tags the gallery already uses, most-used first, with how often each is used, and offers a new tag when nothing matches — no more commas in a text field. Typing a tag the photo already has in other capitals does not add it twice.
- **Removing tags quickly no longer brings them back.** Taking two tags off a photo one after the other removed both, but the panel could show them again a moment later, until the photo was reopened. Each change reloads the gallery behind the photo, and an older reload arriving last put the old tags back on screen. The panel now keeps what it saved.
- **Control panel → Maps reads more clearly.** Offline maps shows a bar of the space used against the limit, with the share in percent. What is already on disk is labelled **Space used**, and the place-names build sits on its own strip with **Update** beside it. The Countries and Towns rows carry icons, their buttons line up on the right, and **Remove** is red. With a towns database added, Countries now says it is not needed rather than "Not installed" — the towns database answers countries too. A saved OpenRouteService address no longer opens **Use your own routing server** as though it were a server of your own.
...
- **Dates are Day, Month and Year, and only the year is needed.** Birth, death, marriage, divorce and life-event dates used to be one box that expected `1924-05-17`, a shape nobody writes by hand. Now each date has a day box, a month list in your own language and a year box, and a year alone is still a whole answer. A day without its month, a month without its year, or the 31st of April is pointed out on the part to fix and stops the save, instead of being quietly saved as a different date.
- **Places are found as you type, and a found place goes on the map.** Birth and death each sit in their own panel of Date and Place. The place box still takes anything, a country, a parish, a village no map knows, but it now also offers the places the tree already uses, spelled the way they were written before, and towns from the server's own place list, so nothing typed leaves the house. Picking one saves its map pin with the words, and changing the words afterwards drops the pin, which named the old words. The arrow on the box lists every place the tree already has.
- **A gender chosen by mistake can be taken back.** Edit person offered only Female and Male, so once one was picked there was no way back to not knowing. **Unknown** is now the third choice.
- **A biography can be formatted.** **Bio / notes** has the same bold, italic, heading, list, quote and link buttons as a story's text, and the profile shows the biography formatted, with its line breaks kept, so biographies already written read as they did. A GEDCOM export leaves the formatting out, because other genealogy programs would show the marks as symbols, and keeps the words and line breaks.
...
- **App storage is one switch.** The Photo Inbox, the library of what the family makes in the app, slideshow renders and kept maps always go together, so they are switched on together, off on a new install, **in system data** with one click or in a folder of your choosing. Free disk space shows where you choose. The four parts are listed with what each is for rather than switched one by one; a part that sits somewhere else offers **Move in**, and **Change** moves the whole of it to another folder as tasks. Switching off is refused, with the reason, while it would lose something: photos still waiting in the Inbox, files in App files, their items in the Recycle Bin, a scan or a render in progress. While it is off, recording, uploading, slideshow movies and kept maps say so and point an admin to the Storage page. Upgrading moves nothing: an install that used any of it has App storage switched on where things already are.
- **The Photo Inbox and App files are system libraries.** The app makes them, keeps their names, and leaves them off the Libraries page and out of the gallery's library filter. There is one Inbox now, and while it is empty members don't see it anywhere. The Recycle Bin's location moved to the Recycle Bin page, where it belongs.
- **The Inbox has reviewers instead of library access.** Admins always review. **Reviewers** on the Inbox's row names anyone else, a person, a group or everyone, as **Can add details** (dates, places, notes, people, voice notes) or **Can keep or discard** (all that, plus Keep, Discard and drop links). Nobody else sees anything of the Inbox. Whoever could see the Inbox before the update keeps it: its managers can keep or discard, and everyone else who had access can add details.
- **A file the app keeps is seen by whoever can see what it belongs to.** App files used to be one library with one access setting, so a recording made for a private story was visible to anyone who could open that library. Now a story recording is heard by whoever can see the story, a voice note by whoever can see its photo, a slideshow's movie by whoever can see the slideshow, and family-tree photos and uploaded music by every member; a file nothing owns is for admins only. There is nothing to set. If App files was an ordinary photo library before 4.6, its photos in folders the app did not make now belong to nothing and only admins see them: its row on the Storage page counts them and says to move their folders to another library.
- **Maps is one page of four cards.** Offline maps, Photo place names, Sign-in locations and Road routes each get a card with a switch, where the data comes from, how it gets here and the space it takes; switching on says what will be downloaded and where, switching off what it frees. The kept-maps limit is chosen on its card, from 100 MB to 2 GB. Sign-in locations can add an optional towns database by upload or link, with pointers to the free ones, and sign-in towns read in your language when place names are on. The gallery gained a **Places** view, towns grouped by country, and the setup guide a Maps step showing the same cards.
- **Fixes.** The viewer showed no town for a photo opened from a grid. Outgoing mail runs on nodemailer 10 and the interface on React 19.3.

## 4.5.0 — Photos know where they were taken

- **A photo with a location now knows the town it was taken in.** Until now a photo's location was a pin and a pair of coordinates: the viewer could show where on the map, but not what the place was called unless someone typed it in. Turn on **Named places** under Maps › Setup and the viewer shows "Verona, Veneto, Italy" where the place goes — in each person's own language, so the same photo reads **Минск** to a Russian reader and **Minsk** to an English one. What a person wrote about the place always comes first; the named place sits under it, since "the dacha" and the town it is near are both worth knowing.
- **A Places filter in the gallery.** With named places on, **Filter** gains a **Places** list — the towns your photos were taken in, most photographed first — so every photo from one trip or one town is a click away, where before the only location filter was "has a location" or "has none".
- **Worked out on your server; no location leaves the house.** Turning it on downloads the free GeoNames place list once (about 220 MB) and builds a 27 MB database from it in the background, which takes a few minutes; the Setup page and the Tasks page follow it. Every photo is named from that database, never by asking an online service where it was taken. The rules were checked against real places: standing in Times Square names Manhattan rather than the town across the river that happens to be nearer, a photo in a village is not credited to the city down the road, the Eiffel Tower is in **Paris** rather than one of its numbered districts, and a point far from any town — at sea, deep in a forest — gets no name rather than a wrong one.
- **A moved pin is named again at once, and names go when the database does.** Drag a photo's pin somewhere else and it takes the new place's name straight away, in the viewer and in the filter. **Update** on Maps › Data rebuilds from GeoNames' latest list while the names already shown stay put, and **Remove** deletes the database and every name with it — the photos' locations, and anything people typed as the place, are not touched.

## 4.4.0 — Maps that can stay at home

- **Maps are drawn afresh, and follow your theme and your language.** Every map in the app — the gallery's Map view, a photo's location, the place picker, story maps and routes, the Dashboard's sign-in locations — now comes from OpenFreeMap, drawn in the browser rather than delivered as ready-made pictures. The old pictures had one look and their place names painted in: a dark theme still got a bright map, and a Russian reader got whatever spelling the picture happened to carry. Now a dark theme gets a dark map, and switches the moment the theme does; place names follow the language of the interface — **Минск** in Russian, **Minsk** in English — falling back to the local name where no translation exists. Maps stay sharp at any size, and on a small map the credit folds away behind an ⓘ button instead of covering the picture. A browser that cannot draw these maps says so where the map would be.
- **Maps can now be kept on this server — and while they are, the map service no longer sees where you look.** Until now every map on every page asked the map service directly for the part of the world it was showing, so the service could follow which places were being looked at; on the Dashboard's Locations view, the map it drew outlined where this household signs in from. Turn on **Maps on this server** and every map comes through your server instead, is kept (up to 200 MB), and still draws when the internet is down for any place already seen. It is off until you turn it on — maps work exactly as before without it — and guests on a shared story see its maps the same way you do.
- **Maps have a place of their own in the control panel: Maps › Setup, Data and Routing.** **Setup** lists what the server keeps for maps and what each costs on disk, and walks you through turning things on: what to keep, where it goes, and fetching it. Turning something off asks first and says exactly what it will delete. **Data** holds the sign-in location databases, which used to be managed from the Locations view, and can now remove one. **Routing** is what used to be **Settings › Maps**; old links to it still work.
- **Sign-in locations are now something you switch on.** The country database is fetched only when you ask for it, from **Maps › Setup**. Until then the Locations map still draws, with a note saying what it is missing and a button that takes you there. Databases you already have are found and used as before.
- **Map data is a room on the Storage page.** Kept maps live in **Map data** inside App storage as soon as there is one, and move with the rest of App storage — as a task on the Tasks page — if you change where it lives. Nothing is moved or fetched on upgrade: with map caching off, the room is empty.

## 4.3.0 — A small backup that keeps what cannot come back

- **A minimal backup now carries the pictures nothing else can bring back.** It used to be the database and the two-factor key alone, on the reasoning that covers can always be made again — and most can: a photo's preview, a video's poster and every face the app has cut out are rendered from your own files, and a scan makes them afresh the moment one is missing. But a cover you uploaded or had the app fetch, a series cover, an author's portrait, a category tile, a family-tree portrait: those exist nowhere but the thumbnail store, and no amount of re-scanning brings them back — a restore from a minimal backup left them blank for good. Those pictures now travel in it, and nothing else does, so the small nightly backup stays small: on a library of forty books and two hundred photos it grew to 7 MB where the full one is 43 MB. A **full** backup is unchanged — it still takes the whole thumbnail store, rendered previews included, so a restore has nothing left to re-render.
- **Each kind of backup keeps its own number of copies.** One count covered all three before, which forced a choice nobody wants to make: keep a fortnight of the small nightly backups and you kept a fortnight of the big weekly ones too. The number now sits under the time in each backup's row on **Maintenance › Backup** — `Keep 14` beside the nightly minimal one, `Keep 2` beside the weekly full one — and each kind is pruned entirely on its own, so neither can push the other out. Quick database copies get a count of their own below the two rows; they have no schedule because nothing ever makes one on a timer. Whatever you had set is kept, and now applies to every kind separately. The automatic copies taken before an upgrade are still counted apart from all of it, and still keep their own newest two.
- **A backup file now says what it is, inside.** Until now the only way to tell a full backup from a minimal one was the file name — which is fine until you rename it, and no use at all now that both kinds carry pictures. Every backup archive carries a small `backup.json` naming its kind, what it holds and the version that wrote it, so a backup uploaded from your computer is filed correctly whatever it arrived called. Older backups still read the way they always did.
- The Backup page also gained the icon every other page in the control panel opens with.
...
## 3.90.0 — The folder can take the room's name
...
## 3.89.0 — Three kinds of backup, on the same schedule as everything else
...
## 3.88.0 — Renders take App storage
...
## 3.87.1 — The Library link is back
...
## 3.86.0 — App files, and a folder that can change library
...
## 3.85.0 — Placed photos stay placed, and the replaced originals are seen
...
## 3.84.2 — The replaced originals move with the bin
...
## 3.84.1 — What the app keeps for itself stays out of the gallery
...
## 3.84.0 — Every room moves the same way
...
## 3.83.2 — The book remembers where you left
...
## 3.83.1 — Pictures of the Storage page
...
## 3.83.0 — App storage moves house
...
## 3.82.2 — The same player for a guest
...
## 3.82.1 — The bars are plain again
...
## 3.82.0 — One recorder, one player
...
## 3.81.0 — A photo's details, in three tabs
...
## 3.80.1 — A picture of the library picker
...
## 3.80.0 — One place for the app's storage
...
## 3.78.0 — Asked once, at the start
...
## 3.77.0 — One folder for the app's own things
...
## 3.75.0 — Ask someone, from anywhere
...
## 3.72.0 — One library for what the app makes
...
## 3.71.0 — Ask someone what they know
...
## 3.70.2 — Passkeys, new parts underneath
...
## 3.70.1 — A thumbnail that would not update
...
## 3.70.0 — Keep knows your folders, and a scan survives a bad file
...
## 3.69.1 — The small dropdowns too
...
## 3.69.0 — One dropdown, everywhere
...
## 3.68.0 — The Inbox knows a print scanned twice
...
## 3.66.1 — The New story dialog, pictured
...
## 3.65.3 — A picture on almost every page
...
## 3.65.2 — A book in two parts is one book
...
## 3.65.0 — A story is news when it is published
...
## 3.64.1 — A checkbox back where it belongs
...
## 3.64.0 — A handful of photos at once
...
## 3.62.0 — Tell the scanner how your shelves are arranged
...
## 3.61.0 — The way you actually went
...
## 3.60.0 — One way to hand anything on
...
## 3.59.0 — Housekeeping you don't have to think about
...
## 3.58.0 — A map that shows the way you went
...
## 3.57.3 — A backup brings your two-factor with it
...
## 3.57.2 — A review shows the book it is about
...
## 3.57.1 — More than two hundred at once
...
## 3.57.0 — The better copy of the same photograph
...
## 3.55.1 — The same story, however it is read
...
## 3.54.2 — Room to change your mind
...
## 3.54.1 — Covers that fill themselves in
...
## 3.53.1 — The guides have pictures again
...
## 3.50.2 — Right where you left it
...
## 3.47.0 — The guest gets the good seats
...
## 3.40.1 — A line that could stop the house
...
## 3.40.0 — Only what you searched for
...
## 3.39.0 — A line about the person
...
## 3.38.0 — The whole branch at once
...
## 3.36.0 — Which one is your author?
...
## 3.34.0 — A library of quotes, not a list
...
## 3.33.1 — Tabs that mind their own branch
...
## 3.31.1 — Audiobooks and Ebooks catch up
...
## 3.29.0 — Night theme with a space program
...
## 3.28.2 — One photo picker, everywhere
...
## 3.28.1 — The memory card earns its place
...
## 3.28.0 — A home page that is different every day
...
## 3.27.0 — Your year, cut from the photos you liked
...
## 3.26.0 — Slideshow movies open like films now — and end like them
...
## 3.25.1 — Emptying the bin now has to be meant
...
## 3.25.0 — The recycle bin stops emptying itself
...
## 3.24.0 — Author pages, rebuilt — with a website, a location, and an editor that says what it will do
...
## 3.23.0 — Lock a folder, and nothing inside it can be deleted from the app
...
## 3.22.0 — A video filmed sideways can be turned upright, just like a photo
...
## 3.21.0 — Tell the app which machine your proxy is, and forged addresses stop being possible
...
## 3.20.2 — A backup that takes its time is no longer reported as a failure
...
## 3.20.1 — The reverse-proxy setting keeps meaning what it says
...
## 3.20.0 — A person's relatives read as a family tree, and every one of them is named
...
## 3.19.0 — The household can pass things to each other, and talk about them where they are
...
## 3.18.0 — The same settings never score lower at home than on the internet
...
## 3.17.1 — A home-only server is graded only on what matters at home
...
## 3.17.0 — Security grades itself — a protection level, and a policy table that says what's weak
...
## 3.16.0 — Logs you can narrow, sort, open and export — and Tasks joins the dashboard
...
## 3.15.0 — A System tab that answers "is the server well?", and Statistics becomes the Libraries tab
...
## 3.14.0 — One Activity tab, with a date range of its own
...
## 3.13.0 — Sign-ins becomes the one place for devices and sessions
...
## 3.12.0 — A real map of sign-ins, and a Sign-ins page that answers who, where and from what
...
## 3.11.1 — Two-factor sign-ins now appear on the dashboard
...
## 3.11.0 — The dashboard opens on sign-ins, with pages for devices and where people connected from
...
## 3.10.5 — An email address is no longer refused for the space around it
...
## 3.10.4 — The Listen button stands out, and every icon is freshly drawn
...
## 3.10.3 — Stronger two-factor secrets — authenticator users need to set theirs up again
...
## 3.10.2 — The System page grows into a dashboard, and the library starts counting what gets played
...
## 3.10.1 — Toolbars that fit your phone, and photos that open without their notes already up
...
## 3.10.0 — It takes a code to drop two-factor, and shared photos forget where they were
...
## 3.9.0 — Blocks that can stay for good, and deletions that stay home
...
## 3.8.1 — A blocked visitor gets a real page, and expired blocks say so
...
## 3.8.0 — A second factor from outside, and blocks that say what they counted
...
## 3.7.6 — Book actions move up top, and tags become tinted pills
...
## 3.7.5 — Search folders by name, and catch a folder copied into itself
...
## 3.7.4 — See and change where deleted files go, from the Recycle Bin itself
...
## 3.7.3 — Backups carry your covers again, and duplicate cleanup stops guessing wrong
...
## 3.7.2 — Restore just the database, and find out why a sign-in really failed
...
## 3.7.1 — Restore a backup bigger than 2 GB, and get a locked-out account back
...
## 3.7.0 — Set how big your photos are, and reach the rest of the gallery from anywhere in it
...
## 3.6.1 — Choose how long remote linking stays open, and a tidier user list
...
## 3.6.0 — Let one person link one device from outside the house, for one hour
...
## 3.5.0 — Sign a TV in by scanning a code, instead of typing a password with a remote
...
## 3.4.3 — Every open album, slideshow and person looks and works the same way now
...
## 3.4.2 — Gallery's library picker moved into Filter too
...
## 3.4.1 — An open album shows what it can do in one place instead of three
...
## 3.4.0 — One toolbar for Audiobooks, Ebooks and Gallery, and an A–Z that reads Cyrillic
...
## 3.3.3 — A network that blocks the app now says so, instead of looking like a failed sign-in
...
## 3.3.2 — A memory dated a couple of days out no longer goes missing
...
## 3.3.1 — Filter and sort moved down beside the library they narrow
...
## 3.3.0 — A menu of its own for each part of the library, and one header on every page
...
## 3.2.0 — Give the movie's opening card your own words and a photo behind them
...
## 3.1.2 — A limit of its own on the one address strangers can reach
...
## 3.1.1 — Add a folder of slideshow music in one go
...
## 3.1.0 — Sign in with a fingerprint, and mail that looks like it came from somewhere
...
## 3.0.8 — Told when something is shared with you, and an A to Z for the authors
...
## 3.0.7 — One drawing on the empty family tree, not two
...
## 3.0.6 — Opens faster, and stops sending things you never see
...
## 3.0.5 — A Recycle Bin you can search, and backups from the start
...
## 3.0.4 — See the copies, and say which ones stay
...
## 3.0.3 — Clear out means clear out
...
## 3.0.2 — The setup guide lets you leave
...
## 3.0.1 — A setup guide on first sign-in, and folders you browse to
...
## 3.0.0 — One page for duplicates, a Recycle Bin you can place and time — and a fresh start
...
## 2.22.0 — One folder page, and folders that share only some photos
...
## 2.21.2 — Every copy says which folder and which library it's in
...
## 2.21.1 — A library's root is a place, not a folder to clear out
...
## 2.21.0 — The duplicate pages send a page, not everything they found
...
## 2.20.3 — Nothing deletes out of an external library, and the duplicate page stops re-asking
...
## 2.20.2 — Restore all could bring the server to a halt
...
## 2.20.1 — Where the copies actually are, instead of "Library root"
...
## 2.20.0 — A Utilities section, and a way to undo an emptying you regret
...
## 2.19.1 — "Library root" said two different things, and neither clearly
...
## 2.19.0 — Duplicates moves to Utilities, as one destination with three tabs
...
## 2.18.0 — Duplicate folders, rebuilt as cards
...
## 2.17.2 — The filter box is two tabs, not one long scroll
...
## 2.17.1 — Fixed the blank Duplicate photos page
...
## 2.17.0 — One filter box, and a bulk delete that obeys it
...
## 2.16.2 — The bulk delete button says what it covers
...
## 2.16.1 — Show only the duplicates you can clear without looking
...
## 2.16.0 — Tell it which folder to clear out
...
## 2.15.1 — Backups were being written where an update would lose them
...
## 2.15.0 — Duplicate folders has its own tab
...
## 2.14.0 — Folders already stored elsewhere
...
## 2.13.1 — Duplicate folders says when it found nothing
...
## 2.13.0 — Duplicate folders, not just duplicate photos
...
## 2.12.2 — Folders say how much is in them
...
## 2.12.1 — Photo thumbnails look at faces
...
## 2.12.0 — Everything you've shared, on one page
...
## 2.11.9 — Adding photos to a relative is tidier
...
## 2.11.8 — Long slideshows render a piece at a time
...
## 2.11.7 — Rendering a movie no longer needs a workstation
...
## 2.11.6 — A background job can no longer take the server down with it
...
## 2.11.5 — The title card is back, on every machine
...
## 2.11.4 — Slideshow movies render again
...
## 2.11.3 — A failed movie now says what went wrong
...
## 2.11.2 — The address Google gave you now works
...
## 2.11.1 — One library at a time on Duplicate photos
...
## 2.11.0 — A control panel that answers back
...
## 2.10.0 — Fixing the date and place a camera got wrong
...
## 2.9.0 — Every settings page has an address
...
## 2.8.3 — One more line that wouldn't fit
...
## 2.8.2 — Readable numbers on smaller screens
...
## 2.8.1 — Follow-ups to the new control panel
...
## 2.8.0 — A control panel you can search
...
## 2.7.0 — You decide which copies stay
...
## 2.6.0 — The same photo, saved twice
...
## 2.5.0 — Find the same photo twice
...
## 2.4.0 — The guides now live inside the app
...
## 2.3.2 — Three new guides, and help that matches your version
...
## 2.3.1 — Old photos land on the right date again
...
## 2.3.0 — A family tree you can steer, and photos that land on the right person
...
## 2.2.1 — A security fix for the Back button
...
## 2.2.0 — Two-factor codes by email, and a starting person for the tree
...
## 2.1.1 — Guides for every part of the app
...
## 2.1.0 — Adding photos to the family tree, and a gentler first run
...
## 2.0.0 — Tags across everything, and a fresh foundation
...
## 1.16.0 — Family branches, and a richer timeline
...
## 1.15.0 — The family tree, the way family trees are drawn
...
## 1.14.2 — A richer person profile
...
## 1.14.1 — A redesigned family tree view
...
## 1.14.0 — Family tree: GEDCOM files, life stories, and sources
...
## 1.13.5 — A tidier Gallery on your phone
...
## 1.13.4 — A way to report a security problem
...
## 1.13.3 — Staying on HTTPS, and a sign-in fix for home installs
...
## 1.13.2 — Two small fixes for servers opened to the internet
...
## 1.13.0 — Security alerts: know when someone gets near your account
...
## 1.12.0 — Family tree: map your family and tie it to your photos
...
## 1.11.3 — Clearer activity logs: filters, named shares, and download auditing
...
## 1.11.2 — Play old-format videos, and keep a movie visible after edits
...
## 1.11.1 — Rendered movies now honour "seconds per photo"
...
## 1.11.0 — Slideshow studio: movie title cards, cinematic transitions, and smarter suggestions
...
## 1.10.0 — Share a whole album — live links that stay in step with the album
...
## 1.9.2 — Fix photo/video uploads on Unraid (read-only media path)
...
## 1.9.1 — Uploaded photos and videos are filed into dated folders
...
## 1.9.0 — Gallery slideshows: build, set to music, and export as a movie
...
## 1.8.23 — Gallery slideshow and clearer sub-view navigation
...
## 1.8.22 — Metadata cover thumbnails — fixed for real, no more server hang
...
## 1.8.20 — Shared audiobook player: tighter layout and one-line controls
...
## 1.8.19 — Polish for the shared-audiobook player
...
## 1.8.18 — A fuller shared-audiobook player and domain-aware share links
...
## 1.8.17 — Redesigned category browsing and smarter genre shelving
...
## 1.8.16 — Flat-folder audiobooks, per-folder gallery rescan & missing-photo cleanup
...
## 1.8.15 — Share photos with people, download whole sets, and a reworked album page
...
## 1.8.14 — The selection bar now stays with you
...
## 1.8.13 — Albums, bulk actions, and share links for photo sets
...
## 1.8.12 — Gallery, day by day — a Memories tab, richer filters, and edit-in-place photo details
...
## 1.8.11 — Memories — "On this day" in the gallery and on Home
...
## 1.8.10 — A usable People page — and a way to spot split-up people
...
## 1.8.9 — All of a person's photos — and a much faster gallery
...
## 1.8.8 — Right face on the person avatar
...
## 1.8.7 — Face grouping no longer freezes the site (and is far faster)
...
## 1.8.6 — Interrupted face scans no longer lose their people
...
## 1.8.5 — Clearer connection banner
...
## 1.8.4 — Face scans no longer freeze the site
...
## 1.8.3 — Tasks page fixes: accurate durations, batched rescans, tidier history
...
## 1.8.2 — Security hardening for the photo & video folder view
...
## 1.8.1 — Fixes the 1.8.0 image failing to start
...
## 1.8.0 — Gallery search with advanced filters, BMP photos fixed, slimmer Docker image
...
## 1.7.2 — Face recognition housekeeping: disk cleanup, unreadable photos, leaner nights
...
## 1.7.1 — Face-scan batches visible up front
...
## 1.7.0 — Nightly library scans, a new Tasks page + smarter face grouping
...
## 1.6.1 — Automatic maintenance schedules + tidier face-recognition window
...
## 1.6.0 — Sharper face recognition + live scan progress
...
## 1.5.0 — Scheduled maintenance jobs + control-panel tidy-up
...
## 1.4.2 — A stronger face-recognition engine (+ Credits page)
...
## 1.4.1 — Face recognition: sharper grouping, face thumbnails & fixes
...
## 1.4.0 — People in your photos — tag faces and recognise them automatically
...
## 1.3.9 — Quotes & bookmarks that work in the installed app — even offline
...
## 1.3.8 — Save quotes & highlights from your books
...
## 1.3.7 — Reliable offline ebook reading & tidier collections
...
## 1.3.6 — Layout fixes for tablets & phones
...
## 1.3.5 — A real installed-app experience on your phone
...
## 1.3.4 — Sidebar quick-links & a new Help page
...
## 1.3.3 — Refreshed navigation & consistent tabbed layouts
...
## 1.3.2 — Admin sign-in unlock & clearer auth logs
...
## 1.3.1 — Fix: container could fail to start (two-factor dependency)
...
## 1.3.0 — Security: two-factor sign-in & internet-ready hardening
...
## 1.2.15 — Send to e-reader, sleep timer & email change
...
## 1.2.13 — Ebook metadata, bulk edit & FantLab
...
## 1.2.11 — Book editions & multi-format ebooks
...
## 1.2.10 — One Authors page for everything
...
## 1.2.8 — Smarter audiobook folder scanning
...
## 1.2.6 — Reader & offline screen redesign
...
## 1.2.5 — Default covers, reader access & library fixes
...
## 1.2.4 — Library setup wizard refresh
...
## 1.2.3 — Mobile library & offline polish
...
## 1.2.2 — Faster offline detection
...
## 1.2.1 — Mobile home & app navigation
...
## 1.2.0 — Profile, theme & bookmarks refresh
...
## 1.0.0 — Rebuilt library foundation
...
## 0.27.0 — Home dashboard: Continue & Recently added
...
## 0.26.0 — Audible metadata provider
...
## 0.25.0 — Lookup compare, paste-a-link & manual people
...
## 0.24.1 — m4b scan fix & chapters tab
...
## 0.24.0 — m4b chapter reading & navigation
...
## 0.22.0 — Online metadata lookup, LibriVox & author photos
...
## 0.21.0 — Folder upload & companion files
...
## 0.20.0 — Audiobook upload & delete
...
## 0.19.0 — Control Panel management refresh
...
## 0.17.1 — Track view polish & progress rings
...
## 0.17.0 — Episodic libraries & smarter progress
...
## 0.16.0 — Members, library editing & scan status
...
## 0.15.0 — Create wizard: Upload & Scanning steps
...
## 0.14.0 — Standardized dialogs, buttons & messages
...
## 0.13.0 — Library scanning options & unified creation
...
## 0.12.0 — Unified permissions & library modes
...
## 0.11.0 — Library roles & permissions
...
## 0.10.1 — Audiobook library polish
...
## 0.10.0 — PWA navigation & offline reliability
...
## 0.9.1 — Bookmarks & quick navigation
...
## 0.8.10 — Cleaner install & offline
...
## 0.8.6 — Sign-in QR & show password
...
## 0.8.5 — Lock-screen & car controls
...
## 0.7.1 — Player polish & smarter bulk edit
...
## 0.7.0 — Redesigned audiobook tiles & bulk editing
...
## 0.6.1 — Shared navigation & library UI polish
...
## 0.5.2 — Audiobook browse redesign & testing tools
...
## 0.4.16 — Listening progress & UX polish
...
## 0.4.15 — Special sections & control panel
...
## 0.4.13 — Category management polish
...
## 0.4.11 — Status dashboard & book rescan
...
## 0.4.10 — Audiobook detail polish
...
## 0.4.9 — Bookmarks, My List & encoding fix
...
## 0.4.7 — Popup player & sidecar improvements
...
## 0.4.6 — Scan performance & reliability
...
## 0.4.5 — Unraid scanner hardening
...
## 0.4.3 — Scan reliability & job controls
...
## 0.4.2 — Jobs, Database & library management
...
## 0.3.0 — Library navigation & people
