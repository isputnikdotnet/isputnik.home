...
type SeriesKey = "loginsSuccess" | "loginsFailed" | "uploads" | "downloads" | "deletes" | "played" | "read" | "viewed";
...
function rollingCount(eventsSql: string, sinceDays: number): number {
  const row = db.prepare(`
    ...
    WHERE event IN (${eventsSql}) AND created_at > strftime('%Y-%m-%dT%H:%M:%fZ', 'now', ?)
...
function rollingLikeCount(likePattern: string, sinceDays: number): number {
  const row = db.prepare(`
    SELECT COUNT(*) AS n FROM activity_logs
    WHERE event LIKE ? AND created_at > strftime('%Y-%m-%dT%H:%M:%fZ', 'now', ?)
  `).get(likePattern, `-${sinceDays} days`) as { n: number };
  return row.n;
}

export async function dashboardRoutesPlugin(app: FastifyInstance) {
  app.get("/api/dashboard/summary", { preHandler: app.requireAdmin }, async (request, reply) => {
    ...
    // created_at is ISO text ('YYYY-MM-DDT…Z'), so "on or after that day" is a
    // plain text comparison against the bare date — a timestamp from that day
    // sorts after its own date prefix — and the created_at index serves it.
  ...
  app.get("/api/dashboard/signins", { preHandler: app.requireAdmin }, async (request, reply) => {
    ...
    // not what once could.
    const sessionConditions = ["sessions.revoked_at IS NULL", "sessions.expires_at > strftime('%Y-%m-%dT%H:%M:%fZ', 'now')", "users.deleted_at IS NULL"];
  ...
  // No body, or a body whose latitude is null/absent, is the clear.
  const homeClearSchema = z.object({ latitude: z.null().optional() }).nullable();

  app.put("/api/dashboard/locations/home", { preHandler: app.requireAdmin }, async (request, reply) => {
    if (!parseBody(homeClearSchema, request.body ?? null).error) {
      setHomeLocation(null, request.user!.id);
      return reply.send({ home: null });
    }

    const parsed = parseBody(homeLocationSchema, request.body);
    if (parsed.error) {
