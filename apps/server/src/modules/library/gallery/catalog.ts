...
export function queryGalleryTimeline(userId: string, libIds: string[], opts: GalleryTimelineQuery) {
  ...
  const where: string[] = [`${scope.sql}`, "library_items.deleted_at IS NULL"];
...
export function queryGalleryFolders(userId: string, libIds: string[], parent: string, limit: number, offset: number) {
  ...
  const libArgs = [...scope.params];

  // relative path of each asset within `parent`; only assets at or below `parent`.
  const scopeWhere = cleanParent
    ? `${scope.sql} AND library_items.deleted_at IS NULL AND library_items.folder_path LIKE ?`
    : `${scope.sql} AND library_items.deleted_at IS NULL`;
  ...
  const directWhere = cleanParent
    ? `${scope.sql} AND library_items.deleted_at IS NULL AND library_items.folder_path LIKE ? AND library_items.folder_path NOT LIKE ?`
    : `${scope.sql} AND library_items.deleted_at IS NULL AND library_items.folder_path NOT LIKE ?`;
...
export function searchGalleryFolders(libIds: string[], q: string, limit: number) {
  ...
  `).all(...scope.params) as { p: LibraryItemRow["folder_path"]; n: number }[];
  ...
  const folders = matched.slice(0, limit).map(([folderPath, count]) => {
    const row = coverStmt.get(...scope.params, folderPath, `${folderPath}/%`) as { cover: ItemMetadataRow["cover_storage_key"] | null } | undefined;
    return {
...
export function galleryFacets(libIds: string[], language = "en") {
  if (scopeIsEmpty(libIds)) return { kinds: [], years: [], withGps: 0, people: [], tags: [], cameras: [], places: [] };
  const scope = galleryScopeSql(libIds);
  ...
  `).all(...scope.params) as { v: GalleryDetailRow["kind"]; n: number }[]).map((r) => ({ kind: r.v, count: r.n }));
  const years = (db.prepare(`
    SELECT DISTINCT substr(gallery_details.taken_at, 1, 4) AS y
    FROM library_items JOIN gallery_details ON gallery_details.item_id = library_items.id
    WHERE ${scope.sql} AND library_items.deleted_at IS NULL AND gallery_details.taken_at IS NOT NULL
    ORDER BY y DESC
  `).all(...scope.params) as { y: string | null }[]).map((r) => r.y).filter((y): y is string => Boolean(y));
  ...
  const tags = (db.prepare(`
    ...
    WHERE ${scope.sql} AND library_items.deleted_at IS NULL
    ORDER BY v COLLATE NOCASE
  `).all(...scope.params) as { v: TagRow["display_name"] }[]).map((r) => r.v);
  ...
  `).all(...scope.params) as { v: string }[]).map((r) => r.v);
...
export function queryGalleryPlaces(userId: string, libIds: string[], language: string) {
  ...
  `).all(...scope.params) as { id: number; item_id: string; n: number }[];
...
function placeFacet(libIds: string[], language: string) {
  ...
  `).all(...scope.params) as { id: number; n: number }[];
