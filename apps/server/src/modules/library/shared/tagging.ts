// The cross-type tags engine: tags + the polymorphic `taggables` table every
// taggable entity joins (library items of every type, albums, slideshows, stories,
// family-tree people, quotes), plus the genre → primary-category match the
// book-like scanners share. It used to live in audiobook/categorize.ts, but
// nothing in it is audiobook-specific.
import { nanoid } from "nanoid";
import { db } from "../../../db.js";
import { stmt } from "../../../db/statement-cache.js";
import { BOOK_LIBRARY_TYPES } from "./library-types.js";
import type { CategoryAliasRow, CategoryRow, ItemCategoryRow, LibraryItemRow, TaggableRow, TagRow } from "../../../db/rows.js";
...
type AliasRow = Pick<CategoryAliasRow, "keyword" | "category_id" | "priority">;

function generalOtherId(): string {
  return (stmt("SELECT id FROM categories WHERE key = 'general_other'").get() as Pick<CategoryRow, "id">).id;
}
...
export function matchCategoryId(rawGenres: string[]): string {
  ...
  const aliases = stmt("SELECT keyword, category_id, priority FROM category_aliases").all() as AliasRow[];
...
function upsertTagId(displayName: string): string {
  ...
  const existing = stmt("SELECT id FROM tags WHERE key = ?").get(key) as Pick<TagRow, "id"> | undefined;
  if (existing) {
    return existing.id;
  }
  stmt("INSERT INTO tags (id, key, display_name) VALUES (?, ?, ?) ON CONFLICT(key) DO NOTHING")
    .run(nanoid(16), key, displayName.trim());
  return (stmt("SELECT id FROM tags WHERE key = ?").get(key) as Pick<TagRow, "id">).id;
}
...
export function addEntityTags(entityType: string, entityId: string, displayNames: string[]) {
  ...
  for (const name of displayNames) {
    ...
    stmt("INSERT OR IGNORE INTO taggables (tag_id, entity_type, entity_id) VALUES (?, ?, ?)")
...
export function rematchAllCategories(): number {
  ...
  `).all(...BOOK_LIBRARY_TYPES) as { id: LibraryItemRow["id"]; category_id: ItemCategoryRow["category_id"] | null }[];
  ...
  db.transaction(() => {
    for (const book of books) {
      const tags = (tagsFor.all(book.id) as { name: TagRow["display_name"] }[]).map((t) => t.name);
...
export function setEntityTags(entityType: string, entityId: string, displayNames: string[]) {
  stmt("DELETE FROM taggables WHERE entity_type = ? AND entity_id = ?").run(entityType, entityId);
...
export function getEntityTags(entityType: string, entityId: string): string[] {
  ...
  `).all(entityType, entityId) as { name: TagRow["display_name"] }[]).map((row) => row.name);
}
...
export function entityTagsByIds(entityType: string, entityIds: string[]): Map<string, string[]> {
  ...
  const rows = db.prepare(`
    ...
    ORDER BY tags.display_name COLLATE NOCASE
  `).all(entityType, ...entityIds) as { id: TaggableRow["entity_id"]; name: TagRow["display_name"] }[];
