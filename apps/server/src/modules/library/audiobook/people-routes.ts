...
import {
  lookupPersonByUrl,
  lookupPersonCandidates,
  lookupPersonPhotoCandidates,
  type PersonLookupSource
} from "./person-lookup.js";
import { removeStoredPhotos, writePersonPhoto } from "./person-photo.js";
...
const nameQuerySchema = z.object({ name: z.string().optional() });

// An unknown source reads as "all" below, as it always has.
...
// The person a by-name route acts on, from ?name=. When it is malformed or
// blank this has already answered 400 and returns null — the handler just
// returns the reply.
function personNameFromQuery(request: FastifyRequest, reply: FastifyReply): string | null {
  const parsed = parseQuery(nameQuerySchema, request.query);
  if (parsed.error) {
...
export async function audiobookPeoplePlugin(app: FastifyInstance) {
  ...
  app.get("/api/library/people/by-name", { preHandler: app.authenticate }, async (request, reply) => {
    const name = personNameFromQuery(request, reply);
    if (name === null) return reply;
  ...
  app.get("/api/library/people/by-name/lookup", { preHandler: app.authenticate }, async (request, reply) => {
    const parsed = parseQuery(lookupQuerySchema, request.query);
    if (parsed.error) {
