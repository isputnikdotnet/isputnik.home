...
export function registerBrowseRoutes(app: FastifyInstance) {
  ...
  // query below must carry libraries.id — the legacy owner/visibility columns alone
  // are no longer enough for canUserAccessLibrary.
  interface AccessLibFields { library_id: string; owner_id: string | null; owner_type: string | null; visibility: string }
  ...
  app.get("/api/library/categories", { preHandler: app.authenticate }, async (request) => {
    ...
    const rows = db.prepare(`
      SELECT book_metadata.category_id AS category_id, libraries.id AS library_id, libraries.owner_id, libraries.owner_type, libraries.visibility
      FROM books
    ...
    for (const row of rows) {
      if (!canUserAccessLibrary({ id: row.library_id }, user.id, user.role)) continue;
  ...
  app.get("/api/library/categories/:key/books", { preHandler: app.authenticate }, async (request, reply) => {
    ...
    const rows = db.prepare(`
      SELECT
        ...
        libraries.id AS library_id, libraries.owner_id, libraries.owner_type, libraries.visibility,
  ...
  app.get("/api/library/tags", { preHandler: app.authenticate }, async (request) => {
    const user = request.user!;
    const rows = db.prepare(`
      SELECT tags.id AS tag_id, tags.display_name AS name, libraries.id AS library_id, libraries.owner_id, libraries.owner_type, libraries.visibility
      FROM taggables
