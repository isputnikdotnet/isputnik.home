...
export function timeAgo(value: string): string {
  ...
  // Module-level helper, so i18n directly (no hook); count keys carry the plurals.
  const ago = (span: string) => i18n.t("common:time.ago", { span });
  const minutes = Math.floor(seconds / 60);
  if (minutes < 60) return ago(i18n.t("common:time.minutes", { count: minutes }));
  const hours = Math.floor(minutes / 60);
  if (hours < 24) return ago(i18n.t("common:time.hours", { count: hours }));
  const days = Math.floor(hours / 24);
  if (days === 1) return i18n.t("common:time.yesterday");
  if (days < 7) return ago(i18n.t("common:time.days", { count: days }));
